package main.java.taskManager.manager;
...
public class TaskManager {
    ...
    public boolean addTask(Task task) {
        String sql = "INSERT INTO task_manager.tasks('task_description','status','deadline','assigned_user_id')" +
                "VALUES (?,?,?,?)";
        try {
    ...
    public boolean changeUser(Task task, long id){
        String sql = "UPDATE task_manager.tasks SET assigned_user_id = ? WHERE assigned_user_id = ?";
        try{
            PreparedStatement statement = connection.prepareStatement(sql);
            statement.setLong(1,id);
